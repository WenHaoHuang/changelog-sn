...
function mergeConfig(options, context, gitRawCommitsOpts, parserOpts, writerOpts) {
  ...
  return Q.allSettled([configPromise, pkgPromise, semverTagsPromise(options), gitRemoteOriginUrlPromise]).spread(function(configObj, pkgObj, tagsObj, gitRemoteOriginUrlObj) {
    ...
    if (options.pkg) {
      if (pkgObj.state === 'fulfilled') {
        if (options.pkg.path) {
          ...
          pkg = pkgObj.value.packageJson || pkgObj.value.pkg || {}
        }
    ...
    if (pkg) {
      ...
      const changelogSnCfg = pkg.config || {}
      const changelogSnObj = changelogSnCfg['changelog-sn'] || {}
      context.pkgSnData = changelogSnObj
      if (changelogSnObj.dateFormat) {
        options.transform = function(commit, cb) {
          if (_.isString(commit.gitTags)) {
            var match = rtag.exec(commit.gitTags)
            rtag.lastIndex = 0
          ...
          if (commit.committerDate) {
            commit.committerDate = dateFormat(commit.committerDate, changelogSnObj.dateFormat, false)
          }

          cb(null, commit)
        }

        context.date = dateFormat(new Date(), changelogSnObj.dateFormat, false)
      }
    }
        ...
        finalizeContext: function(context, writerOpts, filteredCommits, keyCommit, originalCommits) {
          ...
          commitGroups.forEach(commitGroup => {
            ...
            commits.forEach(commit => {
              const index = scopes.findIndex(v => v.scope === commit.scope)
              if (index >= 0) {
                scopes[index].commits.push(commit)
              } else {
                scopes.push({ scope: commit.scope, commits: [commit] })
              }
            })
            commitGroup.scopes = scopes
          })
          context.commitGroups = commitGroups
